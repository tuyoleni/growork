...
const fields: {
    ...
    keyboardType?: 'default' | 'email-address' | 'numeric' | 'url' | 'number-pad' | 'phone-pad';
}[] = [
        ...
        { label: 'Profession *', field: 'profession', placeholder: 'e.g. Software Engineer' },
        { label: 'Bio', field: 'bio', placeholder: 'Short bio...', multiline: true, numberOfLines: 4 },
        ...
        { label: 'Experience Years', field: 'experience_years', placeholder: 'Years', keyboardType: 'numeric' },
        { label: 'Education', field: 'education', placeholder: 'Your education' },
        { label: 'Skills (comma separated)', field: 'skills', placeholder: 'e.g. JavaScript, React, Python' },
    ];
            ...
            <View style={[styles.header, { borderBottomColor: borderColor }]}>
                {editing ? (
                    ...
                    >
                        {loading ? (
                            ...
                            <ThemedText style={[styles.headerButtonText, { color: tintColor }]}>Save</ThemedText>
                        )}
                    </Pressable>
                ) : (
                ...
                {!editing ? (
                    <Pressable onPress={() => setEditing(true)} style={styles.headerButton}>
                        <ThemedText style={[styles.headerButtonText, { color: tintColor }]}>Edit</ThemedText>
                    </Pressable>
                ) : (
                    <Pressable onPress={() => setEditing(false)} style={styles.headerButton}>
                        <ThemedText style={[styles.headerButtonText, { color: '#666' }]}>Cancel</ThemedText>
                    </Pressable>
                )}
            </View>
                    ...
                    <View style={styles.formView}>
                        ...
                        {fields.map((f) => (
                            <View key={f.field} style={styles.fieldRow}>
                                <ThemedText style={[styles.fieldLabel, { color: mutedTextColor }]}>{f.label}</ThemedText>
                                <ThemedInput
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: StyleSheet.hairlineWidth, height: 44 },
    headerTitle: { fontSize: 17, fontWeight: '600' },
    headerButton: { paddingHorizontal: 12, paddingVertical: 8, minWidth: 60, alignItems: 'center' },
    ...
    input: { fontSize: 16, paddingVertical: 12, paddingHorizontal: 16, borderWidth: 1, borderColor: '#e0e0e0', borderRadius: 8, backgroundColor: '#fff' },
